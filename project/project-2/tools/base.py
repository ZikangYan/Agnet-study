...
def tool_action(name: str = None, description: str = None):
    ...
    作用是 为一个普通方法添加元数据或将其注册到某个系统中，以便框架能够识别并将其作为一个可调用的“工具动作”。
    ...
    def decorator(func: Callable):
        ...
        func.__tool_description = description
        return func
    return decorator
...
class Tool(ABC):
    ...
    def __init__(self, name: str, description: str, expandable: bool = False):
        ...
        self.expandable = expandable  # 这个可扩展性可以多了解一下
    ...
    def get_expanded_tools(self) -> Optional[List['Tool']]:
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            """ 返回这个 类中的所有方法和方法对象，self==Tool这个类"""
...
class AutoGeneratedTool(Tool):
    """自动生成工具 - 从方法签名和deocstring中自动提取参数"""
    ...
    def _parse_parameters(self) -> List[ToolParameter]:
        ...
        # 可以用于检查参数数量、类型、默认值，或在不调用函数的情况下模拟调用
