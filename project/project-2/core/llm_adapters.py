...
class BaseLLMAdapter(ABC): # 注意这种抽象类。什么时候采用抽象类？-> 好多子类要实现这个方法的时候使用
    """LLM适配器基类"""

    def __init__(self, api_key: str, base_url: str | None, timeout: int, model: str):
        self.api_key = api_key
        self.base_url = base_url
        self.timeout = timeout
        self.model = model
        self._client = None
        self._async_client = None # 这个是什么

    # 定义抽象方法,这个写了@abstractmethod , 说明子类必须实现
    @abstractmethod
    def create_client(self) -> Any: # 写函数的时候做好写期待的返回类型
        """ 创建客户端实例"""
        pass

    def create_async_client(self) -> Any: # 子类可以选择实现，这个是创建异步的客户端实例
        """创建异步客户端实例，用户可选"""
        return None
    ...
    #声明一个异步函数（协程），调用时不会立即执行，而是返回一个协程对象
    #允许一个函数在执行过程中主动让出控制权，以便其他任务在同一线程中并发执行。
    #这里先理解他时线程和携程的一个调度方案
    async def astream_invoke(self, messages: List[Dict], **kwargs) -> AsyncIterator[str]:
        ...
        """
        queue = asyncio.Queue() # 用于线程和协程之间进行数据传递
...
# ------- ------ ------- ------ ----- 上面都在进行初始化和进程调度 | 下面是对于不同厂家的适配------------------------------

class OpenAIAdapter(BaseLLMAdapter):
    """ 
    用于兼容OpenAI 调用的接口模式
    你想用Qwen的音频模型这样的有单独情况的，需要单独搞
    """
    ...
    def invoke(self, message: List[Dict], **kwargs) -> LLMResponse:
        """非流式调用"""
        if not self._client:
            self._client = self.create_client() # 创建客户端
        
        # 执行时间记录
        start_time = time.time()

        # 这里有一个编程的小问题，什么时候用try什么时候不用try
        try:
            ...
            latency_ms = int((time.time() - start_time) * 1000) # 毫秒级的延时

            # 提取相关的推理内容
            choice = response.choices[0] # 这里就是返回结构化的推理类容
            content = choice.message.content or ""
            reasoning_content = None

            # 如果是带有Thinking 的推理模型要做特殊的处理
            if self._is_thinking_model(self.model):
                # OpenAI o1系列：reasoing_content 在message中
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            response = self._client.chat.completions.create(
                model=self.model,
                messages=messages,
                stream=True, # 就这里不太一样，其他都是和非流式调用是一致的
                **kwargs
            )
