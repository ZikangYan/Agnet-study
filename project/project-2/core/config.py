...
from pydantic import BaseModel # pydantic 数据验证项目

# 在这个小节中我要学习啥？主要核心是对于一个Agents来说有哪些是要进行配置的
"""
有许多的配置，需要我们这里去学习
"""
class Config(BaseModel):
    ...
    compression_threshold: float = 0.8  # 压缩阈值（0.8 = 80%时触发压缩）-> 对谁进行压缩
    ...
    summary_temperature: float = 0.3  # 摘要生成温度（更确定性） 这个需要自己看，但是摘要的话就不要进行发散了

    # 工具输出截断配置（啥意思，为什么要进行工具输出截断）
